...
| PAR14-RMA-STOCK-001 | V1 | Estoque | `page/detalhes.php` (checkbox `marcarestoque`) | `temas.v1.rma.show` rodape | Checkbox real; label `O ITEM E DO ESTOQUE` / `ITEM NAO E DO ESTOQUE` | Checkbox real + mesmos textos (`data-texto-true/falso`); persistencia POST/PUT | Tipo/label equivalentes; render visual difere: o Legacy esconde o input nativo (boundingBox null) e desenha o check via `label > i`, o novo mostra o checkbox nativo 13x13 | E/C | Medio | [R] | Browser (ParidadeCreditoEstoqueRuntime) | Medido 2026-09-10: falta reproduzir o check custom do Legacy |
| PAR14-RMA-CREDIT-001 | V1 | Credito | `page/detalhes.php` (checkbox `creditodisponivel`) | `temas.v1.rma.show` rodape | Checkbox real; label `CREDITO DISPONIVEL` / `MARQUE P/ VALIDAR CREDITO` | Checkbox real + mesmos textos | Tipo/label equivalentes; mesmo gap visual do estoque (input nativo x check custom do Legacy); relacao com `GERADO CREDITO` nao re-testada | E/C | Medio | [R] | Browser (ParidadeCreditoEstoqueRuntime) | Medido 2026-09-10: falta reproduzir o check custom |
...
| PAR15-RMA-STOCK-001 | V2 | Estoque | `page/rma.php` select `marcarestoque` (Nao/Sim), 3a coluna | `_form_detalhe` select `marcarestoque` | Select Nao/Sim, label `E um produto do estoque ?` | Select Nao/Sim, mesmo label | Equivalente | E | Medio | [x] | Browser (ParidadeCreditoEstoqueRuntime) | MEDIDO: select Nao/Sim, label igual, altura 30px nos dois lados (corrigido de 34px no CSS V2) |
| PAR15-RMA-CREDIT-001 | V2 | Credito | `page/rma.php` select `creditodisponivel` (Nao/Sim) | `_form_detalhe` select `credito_disponivel` | Select Nao/Sim, label `E credito disponivel ?` | Select Nao/Sim, mesmo label | Nome do campo diverge (`creditodisponivel` x `credito_disponivel`) | E | Baixo | [x] | Browser (ParidadeCreditoEstoqueRuntime) | MEDIDO: select Nao/Sim e label iguais nos dois; divergencia de nome mantida e ligada ao backend |
...
| 3 | PAR14-RMA-STOCK-001 / PAR14-RMA-CREDIT-001 / PAR15-RMA-STOCK-001 / PAR15-RMA-CREDIT-001 | Gaps de credito/estoque medidos no runtime (V2 corrigido; V1 com check custom pendente) | [R] |
