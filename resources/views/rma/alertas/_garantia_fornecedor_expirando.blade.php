{{--
    ...
    Janela: 336 < dias_decorridos < 365. [CONFIRMADO-15.8.1]
--}}
<table class="Tabelinha-Table tabela-alerta-garantia-fornecedor-expirando">
    <thead>
        <tr class="SuperTr">
            <th style="width:8%">ENTRADA</th>
            <th style="width:7%">ORIGEM</th>
            <th style="width:6%">NF C</th>
            <th style="width:6%">T E</th>
    ...
    <tbody>
        @foreach ($rmas as $indice => $registro)
            @php
                ...
                // T E = dias restantes = 365 - dias decorridos desde NF compra
                $diasDecorridos = $dataNfCompra?->diffInDays(today()) ?? 0;
                $diasRestantes = $dataNfCompra !== null ? max(0, 365 - $diasDecorridos) : 0;
            ...
            <tr class="{{ $indice % 2 === 0 ? 'TrZebrada2' : 'TrZebrada1' }}">
                ...
                <td>{{ $diasRestantes > 0 ? $diasRestantes : '' }}</td>
