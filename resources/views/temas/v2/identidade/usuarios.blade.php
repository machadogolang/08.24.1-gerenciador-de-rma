        ...
        <tbody>
            @foreach ($usuarios as $indice => $usuario)
                <tr class="{{ $indice % 2 === 0 ? 'TrZebrada1' : 'TrZebrada2' }}">
                    ...
                    <td>
                        <form method="POST" action="{{ route('identidade.usuarios.update', $usuario) }}" class="form-inline">
                            @csrf
                            @method('PUT')
                            ...
                            <button type="submit" class="btn formSubmit">Salvar papel</button>
                        </form>
                    </td>
                    <td>
                        <form method="POST" action="{{ route('identidade.usuarios.resetar-senha', $usuario) }}" class="form-inline">
                            @csrf
                            <input type="password" class="form-control" name="nova_senha" placeholder="Nova senha" required>
                            <input type="password" class="form-control" name="nova_senha_confirmation" placeholder="Confirmar" required>
                            <button type="submit" class="btn formSubmit">Resetar senha</button>
                        </form>
                    </td>
                </tr>
            @endforeach
