...
@section('conteudo')
    <div class="pagina pagina--dados">
        <div class="pagina__cabecalho">
            <div class="pagina__cabecalho-conteudo">
                ...
                <p class="pagina__resumo">
                    {{ $registros->count() }} {{ $registros->count() === 1 ? 'produto encaminhado' : 'produtos encaminhados' }}
                </p>
            </div>
        ...
        @include('temas.v3.rma.relatorios._menu_relatorios', ['relatorioAtual' => 'rmpe'])
        ...
        <div class="barra-busca">
            <input class="barra-busca__campo" type="search" id="filtro-rmpe"
                placeholder="Filtrar por número, descrição ou modelo..."
                aria-label="Filtrar encaminhados" data-v3-filtro-tabela>
        </div>

        @if ($registros->isEmpty())
            <div class="estado-vazio">
                <p>Nenhum produto encaminhado no momento.</p>
            </div>
        @else
                    ...
                    <tbody>
                        @foreach ($registros as $registro)
                            <tr data-linha-parceiro>
                                ...
                                <td>
                                    <span class="status-badge status-badge--encaminhado">
        ...
        @if (isset($relatorio['informacaoAdicional']))
            <section class="cartao" style="margin-top: 24px;">
                ...
                <form method="POST" action="{{ rota_tema('rmas.relatorios.informacao-adicional.update', ['codigo' => 'RMPE']) }}">
                    @csrf
                    @method('PUT')
